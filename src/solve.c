...
void update_prediction_function(int i){
	...
	//This is the last check in our repetition checking, so we know we have one more unique config
	num_unique_configs++;
...
int solve(){
	...
	while (fringe != NULL){
		...
		if(states_same(curr_state, goal_state)){
			...
			printf("Unique configurations generated by solver: %ld", num_unique_configs);
		...
		for(int i = 0; i < 4; i++){
			...
			//Additionally, we need to update the num_unique_configs, this will be done in update_prediction_function
